...
ImageName="$Registry/persian-web-scrapper/$Container"
ContainerParams=""

if [ $Container == "scrapper" ];then
...
if [ $rebuild -eq 1 ];then
    ...
    cd TLPC && sudo docker build -f docker/Dockerfile  -t ${ImageName}:$NewVersion . 
fi && \
